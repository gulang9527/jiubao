...
from utils import (
    validate_delete_timeout, validate_time_format, validate_interval, 
    get_media_type, get_file_id, format_datetime, format_duration,
    CallbackDataBuilder, KeyboardBuilder
)
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    if await handle_form_input(update, context):
        logger.info(f"消息被表单处理器处理")
        return
    
    # 处理设置输入
    if await handle_settings_input(update, context):
        logger.info(f"消息被设置处理器处理")
        return
...
async def handle_keyword_form_input(update: Update, context: CallbackContext, input_type: str):
    """处理关键词表单输入"""
    message = update.effective_message
...
async def handle_broadcast_form_input(update: Update, context: CallbackContext, input_type: str):
    """处理轮播消息表单输入"""
    message = update.effective_message
    ...
    if not form_data:
        logger.warning(f"用户 {user_id} 处于轮播消息输入模式但无表单数据")
        await message.reply_text("❌ 轮播消息表单数据丢失，请重新开始")
        context.user_data.pop('waiting_for', None)
        return True
    ...
    if input_type == 'broadcast_text':
        ...
        text = message.text
        if len(text) > 1000:
            await message.reply_text("❌ 文本内容过长，最多1000字符")
            return True
        ...
        # 提供继续按钮
        keyboard = [[InlineKeyboardButton("继续", callback_data="bcform_text_received")]]
        await message.reply_text(
            "✅ 已设置轮播消息文本\n\n点击「继续」进行下一步",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
    ...
    elif input_type.startswith('broadcast_') and input_type.endswith('_time'):
        # 处理时间输入（开始时间或结束时间）
        time_type = input_type.split('_')[1]  # 获取 'start' 或 'end'
        time_str = message.text.strip()
        
        # 验证时间格式
        dt = validate_time_format(time_str)
        if not dt:
            await message.reply_text(
        ...
        # 存储时间设置
        form_data[f'{time_type}_time'] = dt
        context.user_data['broadcast_form'] = form_data
        context.user_data.pop('waiting_for', None)
        
        # 提供继续按钮
        keyboard = [[InlineKeyboardButton("继续", callback_data="bcform_time_done")]]
        await message.reply_text(
            f"✅ 已设置{time_type}时间: {format_datetime(dt)}\n\n点击「继续」进行下一步",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return True
        ...
        try:
            ...
            max_interval = 86400 * 7  # 最大间隔7天
            
            if interval < min_interval:
                await message.reply_text(f"❌ 间隔时间不能小于 {min_interval} 秒")
                return True
                
            if interval > max_interval:
                await message.reply_text(f"❌ 间隔时间不能大于 {format_duration(max_interval)}")
                return True
            ...
            # 提供继续按钮
            keyboard = [[InlineKeyboardButton("继续", callback_data="bcform_content_done")]]
            await message.reply_text(
                f"✅ 已设置间隔时间: {format_duration(interval)}\n\n点击「继续」进行下一步",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
            return True
            
        except ValueError:
            await message.reply_text("❌ 请输入有效的数字（秒）")
            return True
    
    return False

async def handle_settings_input(update: Update, context: CallbackContext):
    """处理设置输入，使用SettingsManager"""
    bot_instance = context.application.bot_data.get('bot_instance')
    user_id = update.effective_user.id
    message = update.effective_message
    ...
    if 'stats_min_bytes' in active_settings:
        await bot_instance.settings_manager.process_setting(
            user_id, 'stats_min_bytes', message, 
    ...
    if 'stats_daily_rank' in active_settings:
        await bot_instance.settings_manager.process_setting(
            user_id, 'stats_daily_rank', message, 
    ...
    if 'stats_monthly_rank' in active_settings:
        await bot_instance.settings_manager.process_setting(
            user_id, 'stats_monthly_rank', message, 
    ...
    if 'auto_delete_timeout' in active_settings:
        await bot_instance.settings_manager.process_setting(
            user_id, 'auto_delete_timeout', message, 
...
async def start_broadcast_form(update: Update, context: CallbackContext, group_id: int):
    ...
    try:
        ...
        if 'broadcast' in active_settings:
            ...
            logger.info(f"已清理用户 {user_id} 的旧轮播消息设置状态")
        ...
        if update.callback_query:
            await update.callback_query.edit_message_text(f"❌ 启动轮播消息表单出错: {str(e)[:50]}...")
        else:
            await update.message.reply_text(f"❌ 启动轮播消息表单出错: {str(e)[:50]}...")
        return
...
async def show_broadcast_time_options(update: Update, context: CallbackContext):
    """显示轮播消息时间设置选项"""
    form_data = context.user_data.get('broadcast_form', {})
...
async def show_broadcast_time_preset_options(update: Update, context: CallbackContext, time_type):
    """显示轮播消息时间预设选项"""
    # 构建预设选项
    keyboard = []
...
async def show_broadcast_interval_options(update: Update, context: CallbackContext):
    """显示轮播消息间隔设置选项"""
    form_data = context.user_data.get('broadcast_form', {})
...
async def preview_broadcast_message(update: Update, context: CallbackContext):
    """预览轮播消息效果"""
    form_data = context.user_data.get('broadcast_form', {})
...
async def handle_broadcast_form_callback(update: Update, context: CallbackContext):
    ...
    try:
        ...
        data = query.data
        logger.info(f"处理轮播消息表单回调，数据: {data}")
        parts = data.split('_')
        logger.info(f"解析轮播消息回调数据: {parts}")
        ...
        if action == "cancel":
            ...
            # 显示时间设置选项
            await show_broadcast_time_options(update, context)
            
        elif action == "set_start_time":
            # 设置开始时间
            await show_broadcast_time_preset_options(update, context, "start")
            
        elif action == "set_end_time":
            # 设置结束时间
            await show_broadcast_time_preset_options(update, context, "end")
            ...
            # 设置发送间隔
            await show_broadcast_interval_options(update, context)
            ...
            # 预览轮播消息效果
            await preview_broadcast_message(update, context)
        ...
        logger.error(f"处理轮播消息表单回调时出错: {e}", exc_info=True)
        if update.callback_query:
            try:
                await update.callback_query.edit_message_text(f"❌ 处理操作时出错: {str(e)[:50]}...")
            except Exception as reply_error:

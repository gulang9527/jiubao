    ...
    async def add_message_stat(self, group_id: int, user_id: int, message: Message):
        ...
        try:
            ...
            if (message_size < min_bytes) or (media_type and not count_media):
                logger.debug(f"消息不满足统计条件: size={message_size}, min_bytes={min_bytes}, media_type={media_type}, count_media={count_media}")
                return
                
            # 添加到数据库
            await self.db.add_message_stat(stat_data)
            logger.debug(f"已添加消息统计: group_id={group_id}, user_id={user_id}, size={message_size}")
